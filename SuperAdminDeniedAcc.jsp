...
try{
  connection = DriverManager.getConnection(connectionUrl+database, userid, password);
  statement=connection.createStatement();
  String sql ="Select firstname,lastname,email,phone_number,address_line,status from administrator where status='Denied' UNION Select firstname,lastname,email,phone_number,address_line,status from client where status='Denied'"; 
  int i=0;
  resultSet = statement.executeQuery(sql);
